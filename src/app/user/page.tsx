...
export default function UserDashboard() {
  ...
  const getRecentBookingActions = (booking: any) => {
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-gray-600">
                    You have <span className="font-semibold text-blue-600 text-lg">{profile?.remainingQuota || 0}</span> cylinder(s) remaining this year
                  </p>
                  {profile?.remainingQuota !== undefined && profile.remainingQuota <= 2 && (
                    <p className="text-sm text-yellow-700 mt-2">
                      ⚠️ You're running low on cylinders. Only {profile.remainingQuota} remaining this year.
                    </p>
                  )}
                  {profile?.remainingQuota !== undefined && profile.remainingQuota > 0 && (
                    <button
                ...
                <div className="text-right">
                  <div className="text-4xl font-bold text-blue-600">{profile?.remainingQuota || 0}</div>
                          ...
                          switch (action) {
                            ...
                            case 'repay':
                              return (
                                <button
                                  key="repay"
                                  onClick={() => router.push(`/user/repay/${b.id}`)}
                                  ...
                                  onClick={async () => {
                                    if (confirm('Are you sure you want to cancel this booking?')) {
                                      try {
                                        ...
                                        if (response.ok) {
                                          if (b.paymentMethod === 'UPI' && b.paymentStatus === 'SUCCESS') {
                                            toast.success('Booking cancelled successfully! Payment will be refunded in 5-6 business working days.');
                                          } else {
