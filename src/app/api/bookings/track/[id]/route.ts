...
async function getTrackingHandler(
  ...
  let events = await prisma.bookingEvent.findMany({
    where: { bookingId: id },
  ...
  if (!events || events.length === 0) {
    ...
    const synthesized: { status: "PENDING" | "APPROVED" | "OUT_FOR_DELIVERY" | "DELIVERED" | "CANCELLED"; title: string; ts: Date }[] = [
      { status: "PENDING", title: "Started", ts: booking.requestedAt },
      {
        status: "PENDING",
        title: "Booking Requested",
        ts: addMinutes(booking.requestedAt, 1),
      },
    ];
    if (["APPROVED", "OUT_FOR_DELIVERY", "DELIVERED"].includes(booking.status))
      synthesized.push({
        status: "APPROVED",
    ...
    if (booking.status === "DELIVERED")
      synthesized.push({
        status: "DELIVERED",
        title: "Delivered",
        ts: booking.deliveredAt || addMinutes(now, 4),
      });
