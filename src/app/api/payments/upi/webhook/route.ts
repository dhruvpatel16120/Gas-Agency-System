...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!secret) return NextResponse.json({ success: false, message: 'Webhook not configured' }, { status: 503 });

    const signature = request.headers.get('x-upi-signature') || request.headers.get('X-UPI-Signature');
    ...
    const payment = await (prisma as any).payment.findFirst({ where: { bookingId }, orderBy: { createdAt: 'desc' } });
    ...
    const updated = await (prisma as any).payment.update({
      where: { id: payment.id },
      data: { status, upiTxnId: upiTxnId || payment.upiTxnId }
    });
