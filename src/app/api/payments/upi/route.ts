...
async function getUPIInvoice(request: NextRequest) {
  try {
    ...
    if (booking.paymentMethod !== "UPI")
      return NextResponse.json(
        { success: false, message: "This booking is not using UPI payment" },
        { status: 400 },
      );
      ...
      return NextResponse.json(
        {
          success: false,
          message:
            "Admin UPI ID is not configured or invalid. Please set a valid ADMIN_UPI_ID in environment.",
        },
        { status: 503 },
      );
    }
    ...
    return NextResponse.json({
      success: true,
      data: { booking, payment, adminUpiId: envUpi },
    });
  } catch (error) {
    ...
    return NextResponse.json(
      {
        success: false,
        message: "Failed to prepare payment. Please try again later.",
      },
      { status: 500 },
    );
  }
...
async function confirmUPIPayment(request: NextRequest) {
  try {
    ...
    if (!bookingId || !upiTxnId) {
      return NextResponse.json(
        { success: false, message: "bookingId and upiTxnId are required" },
        { status: 400 },
      );
    }
    ...
    if (booking.status === "CANCELLED")
      return NextResponse.json(
        {
          success: false,
          message: "Booking is cancelled. Payment not accepted.",
        },
        { status: 400 },
      );
    ...
    try {
      ...
      if (email) {
        ...
        void emailModule.sendEmail(email, subject, html);
      }
    } catch (e) {
    ...
    return NextResponse.json(
      {
        success: false,
        message: "Failed to confirm payment. Please try again later.",
      },
      { status: 500 },
    );
  }
...
export const POST = withMiddleware(confirmUPIPayment, {
  requireAuth: true,
