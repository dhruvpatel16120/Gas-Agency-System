...
async function getPartnerHandler(
  ...
  const data = await prisma.deliveryPartner.findUnique({
    where: { id },
...
async function updatePartnerHandler(
  ...
  const exists = await prisma.deliveryPartner.findUnique({
    where: { id },
    select: { id: true },
  });
...
async function deletePartnerHandler(
  ...
  if (activeAssignments > 0)
    throw new ConflictError("Cannot delete partner with active assignments");
  await prisma.deliveryPartner.delete({ where: { id } });
  return successResponse(null, "Partner deleted");
}

export const GET = withMiddleware(getPartnerHandler, {
  requireAuth: true,
  requireAdmin: true,
  validateContentType: false,
});
export const PUT = withMiddleware(updatePartnerHandler, {
  requireAuth: true,
  requireAdmin: true,
  validateContentType: true,
});
export const PATCH = withMiddleware(updatePartnerHandler, {
  requireAuth: true,
  requireAdmin: true,
  validateContentType: true,
});
export const DELETE = withMiddleware(deletePartnerHandler, {
  requireAuth: true,
  requireAdmin: true,
  validateContentType: false,
});
