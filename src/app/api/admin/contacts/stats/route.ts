...
async function getContactStatsHandler(_request: NextRequest) {
  ...
  try {
    ...
    const newContacts = await prisma.contactMessage.count({
      where: { status: "NEW" },
    });
    const openContacts = await prisma.contactMessage.count({
      where: { status: "OPEN" },
    });
    const resolvedContacts = await prisma.contactMessage.count({
      where: { status: "RESOLVED" },
    });
    const archivedContacts = await prisma.contactMessage.count({
    ...
    const contactsByCategory = await prisma.contactMessage.groupBy({
      by: ["category"],
    ...
    const contactsByPriority = await prisma.contactMessage.groupBy({
      by: ["priority"],
    ...
    const contactsWithReplies = await prisma.contactMessage.findMany({
      where: {
    ...
    const monthlyTrends = await prisma.contactMessage.groupBy({
      by: ["createdAt"],
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      const monthCount = monthlyTrends
        .filter((item) => {
        ...
        .reduce((acc: number, item) => acc + item._count.createdAt, 0);
