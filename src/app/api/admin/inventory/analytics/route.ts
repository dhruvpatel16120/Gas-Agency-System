...
async function getInventoryAnalyticsHandler() {
  try {
    ...
    // Calculate metrics
    const totalReceived = stock?.adjustments
      .filter(a => a.delta > 0)
      .reduce((sum, a) => sum + a.delta, 0) || 0;

    const totalIssued = Math.abs(stock?.adjustments
      .filter(a => a.delta < 0)
      .reduce((sum, a) => sum + a.delta, 0) || 0);
...
export const GET = withMiddleware(getInventoryAnalyticsHandler, { requireAuth: true, requireAdmin: true, validateContentType: false });
