      ...
      return NextResponse.json(
        {
          success: false,
          message:
            "One or more selected bookings are already delivered. Service is fulfilled; bulk actions are not allowed.",
        },
        { status: 400 },
      );
    }
    ...
    switch (action) {
      case "approve":
        ...
        if (approveEligible.length === 0) {
          return NextResponse.json(
            { success: false, message: "No eligible bookings to approve" },
            { status: 400 },
          );
        }

        // Check UPI payment status - only allow approval if payment is SUCCESS
        const upiBookingsWithoutSuccessPayment = approveEligible.filter(
          (booking) =>
            booking.paymentMethod === "UPI" &&
            !booking.payments.some(
              (payment) =>
                payment.method === "UPI" && payment.status === "SUCCESS",
            ),
        );
        ...
        const finalApproveEligible = approveEligible.filter(
          (booking) =>
            !(
              booking.paymentMethod === "UPI" &&
              !booking.payments.some(
          ...
          return NextResponse.json(
            {
              success: false,
              message:
                "No eligible bookings to approve after payment validation",
            },
            { status: 400 },
          );
        }
        ...
        try {
          ...
          for (const b of approved) {
            if (b.user?.email) {
              const deliveryDateStr = b.expectedDate
      ...
      case "assign-delivery":
        ...
        if (eligibleForAssign.length === 0) {
          return NextResponse.json(
            { success: false, message: "No eligible bookings to assign" },
            { status: 400 },
          );
        }
      ...
      case "cancel":
        ...
        if (
          ...
          additionalData.reason.trim().length === 0
        ) {
          return NextResponse.json(
        ...
        if (eligibleForCancel.length === 0) {
          return NextResponse.json(
            { success: false, message: "No eligible bookings to cancel" },
            { status: 400 },
          );
        }
        await prisma.$transaction(async (tx) => {
          ...
          await tx.payment.updateMany({
            where: {
              bookingId: { in: eligibleForCancel.map((b) => b.id) },
              status: { not: "SUCCESS" },
            },
          ...
          await Promise.all(
            eligibleForCancel.map((b) => {
              const prefix = b.notes ? b.notes.trim() + "\n" : "";
              return tx.booking.update({
                where: { id: b.id },
                data: {
                  notes: `${prefix}Cancellation reason: ${additionalData!.reason}`,
                },
              });
            }),
          );
...
export const POST = withMiddleware(bulkActionHandler, {
  requireAuth: true,
  requireAdmin: true,
