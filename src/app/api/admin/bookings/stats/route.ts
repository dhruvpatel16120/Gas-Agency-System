...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      if (isDev)
        console.error("DB health check failed for stats endpoint:", dbErr);
      const emptyStats = {
        total: 0,
        pending: 0,
        approved: 0,
        ...
        recentBookings: [] as Array<{ id: string; status: string; quantity: number; userName: string; userEmail: string; createdAt: Date }>,
      };
      return NextResponse.json({
        success: true,
        data: emptyStats,
        message: "DB unavailable; returning default stats",
      });
    }
    ...
    try {
      [total, pending, approved, outForDelivery, delivered, cancelled] =
        await Promise.all([
          ...
          prisma.booking.count({
            where: { ...whereClause, status: "OUT_FOR_DELIVERY" },
          }),
          prisma.booking.count({
    ...
    try {
      ...
      const validDeliveries = deliveredBookings.filter(
        (booking) => booking.deliveredAt && booking.requestedAt,
      ) as Array<{ requestedAt: Date; deliveredAt: Date }>;

      const totalDeliveryTime = validDeliveries.reduce<number>((total, booking) => {
        return (
          total +
          (new Date(booking.deliveredAt).getTime() -
      ...
      if (isDev)
        console.log("Delivery time calculation completed:", {
          averageDeliveryTime,
    ...
    try {
      ...
      paymentMethodDistribution = paymentMethodStats.reduce(
    ...
    // Get recent activity
    let recentBookings: Array<{ id: string; status: string; quantity: number; user: { name: string; email: string }; createdAt: Date }> = [];
    try {
      ...
      })) as Array<{ id: string; status: string; quantity: number; user: { name: string; email: string }; createdAt: Date }>;
      ...
      recentBookings = [] as Array<{ id: string; status: string; quantity: number; user: { name: string; email: string }; createdAt: Date }>;
    }
    ...
    return NextResponse.json({
      success: true,
      data: emptyStats,
      message: "DB error; returning default stats",
    });
  }
}
