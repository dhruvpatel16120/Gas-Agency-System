...
export async function POST(
  ...
  try {
    ...
    if (!latestPayment) {
      return NextResponse.json({ success: false, message: 'No payment to review' }, { status: 400 });
    }
    if (latestPayment.method !== 'UPI') {
      return NextResponse.json({ success: false, message: 'Only UPI payments can be reviewed here' }, { status: 400 });
    }
    ...
    if (action === 'CONFIRM') {
      ...
      if (!reason || !reason.trim()) {
        return NextResponse.json({ success: false, message: 'Reason is required to reject a payment' }, { status: 400 });
      }
    ...
    try {
      await prisma.bookingEvent.create({
        data: {
          ...
          title: action === 'CONFIRM' ? 'UPI payment confirmed' : 'UPI payment issue',
          description:
            action === 'CONFIRM'
    ...
    return NextResponse.json({ success: true, message: 'Payment reviewed', data: updated });
  } catch (error) {
    console.error('Failed to review payment:', error);
    return NextResponse.json({ success: false, message: 'Failed to review payment' }, { status: 500 });
  }
}
