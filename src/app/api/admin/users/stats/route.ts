...
async function statsHandler() {
  const [totalUsers, admins, users, withQuotaLow, totalBookings] = await Promise.all([
    prisma.user.count(),
  ...
  return successResponse({ totalUsers, admins, users, withQuotaLow, totalBookings }, 'User stats');
}
