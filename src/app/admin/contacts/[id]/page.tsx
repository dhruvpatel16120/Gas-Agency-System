...
const statusConfig = {
  NEW: { label: 'New', color: 'bg-blue-100 text-blue-800 border-blue-200', icon: Clock, bg: 'bg-blue-50' },
  OPEN: { label: 'Open', color: 'bg-orange-100 text-orange-800 border-orange-200', icon: AlertCircle, bg: 'bg-orange-50' },
  RESOLVED: { label: 'Resolved', color: 'bg-green-100 text-green-800 border-green-200', icon: CheckCircle2, bg: 'bg-green-50' },
  ARCHIVED: { label: 'Archived', color: 'bg-gray-100 text-gray-800 border-gray-200', icon: Archive, bg: 'bg-gray-50' }
};
        ...
        <div className="px-4 sm:px-0">
          ...
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-gray-600">Manage customer inquiry and responses</p>
              </div>
            </div>
            {data && (
              <div className="flex items-center gap-3">
                <span className={`inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium border ${statusConfig[data.status].color}`}>
          ...
          {loading || !data ? (
            <div className="flex items-center justify-center py-20">
              ...
              <span className="ml-4 text-gray-500">Loading contact details...</span>
            </div>
          ) : (
              ...
              <div className="xl:col-span-3 space-y-6">
                  ...
                  <div className="p-6 border-b border-gray-100 bg-gradient-to-r from-gray-50 to-white">
                    <div className="flex items-start justify-between mb-4">
                      ...
                      <div className="flex items-center gap-2">
                        {data.priority && priorityConfig[data.priority as keyof typeof priorityConfig] && (
                          <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium border ${priorityConfig[data.priority as keyof typeof priorityConfig].color}`}>
                            <Flag className="w-3 h-3 mr-1" />
                            {priorityConfig[data.priority as keyof typeof priorityConfig].label} Priority
                          </span>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="bg-gray-50 rounded-xl p-4 border border-gray-100">
                      <p className="text-gray-800 whitespace-pre-wrap leading-relaxed">{data.message}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                  <div className="p-6 border-b border-gray-100 bg-gradient-to-r from-gray-50 to-white">
                    ...
                    <p className="text-sm text-gray-600">View the complete conversation thread</p>
                </div>
                  <div className="p-6">
                    {data.replies.length === 0 ? (
                      <div className="text-center py-12">
                        ...
                        <p className="text-gray-500">Be the first to respond to this inquiry</p>
                      </div>
                    ) : (
                      <div className="space-y-4">
                        {data.replies.map((reply, index) => (
                          <div key={reply.id} className={`relative ${reply.isAdmin ? 'ml-8' : 'mr-8'}`}>
                            <div className={`flex ${reply.isAdmin ? 'justify-end' : 'justify-start'}`}>
                              <div className={`max-w-2xl rounded-2xl p-4 ${
                                ...
                                <div className="flex items-center gap-2 mb-2">
                                  ...
                                  <span className={`text-xs ${reply.isAdmin ? 'text-purple-200' : 'text-gray-500'}`}>
                            ...
                            {index < data.replies.length - 1 && (
                              <div className={`absolute top-4 w-0.5 h-8 bg-gray-200 ${
                                reply.isAdmin ? 'right-0 translate-x-2' : 'left-0 -translate-x-2'
                              }`}></div>
                            )}
                      </div>
                    ))}
                ...
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                  <div className="p-6 border-b border-gray-100 bg-gradient-to-r from-gray-50 to-white">
                    <h3 className="text-lg font-semibold text-gray-900 mb-2">Send Reply</h3>
                    <p className="text-sm text-gray-600">Your response will be emailed to the customer</p>
                  </div>
                  <form onSubmit={onReply} className="p-6">
                    ...
                    <div className="flex items-center justify-between">
                      <p className="text-sm text-gray-500">
                        Replying to: <span className="font-medium text-gray-700">{data.user.name}</span>
                      </p>
                      <button 
                        disabled={replying} 
              ...
              <div className="xl:col-span-1 space-y-6">
                ...
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                  <div className="p-6 border-b border-gray-100 bg-gradient-to-r from-gray-50 to-white">
                    ...
                    <p className="text-sm text-gray-600">Contact and account details</p>
                  </div>
                ...
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                  <div className="p-6 border-b border-gray-100 bg-gradient-to-r from-gray-50 to-white">
                    ...
                    <p className="text-sm text-gray-600">Manage ticket status and details</p>
                </div>
                  <div className="p-6 space-y-4">
                    {editing ? (
                  ...
                  <div>
                      ...
                      <div className="space-y-4">
                        <div className="flex items-center justify-between">
                          ...
                          <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border ${statusConfig[data.status].color}`}>
                            {getStatusIcon(data.status)}
                            {statusConfig[data.status].label}
                          </span>
                        </div>
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-sm font-medium text-gray-900">{data.category || 'General'}</span>
                        </div>
                        <div className="flex items-center justify-between">
                          <span className="text-sm text-gray-600">Priority</span>
                          {data.priority && priorityConfig[data.priority as keyof typeof priorityConfig] ? (
                            <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium border ${priorityConfig[data.priority as keyof typeof priorityConfig].color}`}>
                ...
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                  <div className="p-6 border-b border-gray-100 bg-gradient-to-r from-gray-50 to-white">
                    ...
                    <p className="text-sm text-gray-600">Timeline and metadata</p>
                  </div>
                  <div className="p-6 space-y-4">
                    <div className="flex items-center justify-between">
                    ...
                    {data.lastRepliedAt && (
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-sm font-medium text-gray-900">{formatDate(data.lastRepliedAt)}</span>
                      </div>
                    )}
                    <div className="flex items-center justify-between">
                    ...
                    {data.relatedBookingId && (
                      <div className="flex items-center justify-between">
                        <span className="text-sm text-gray-600">Related Booking</span>
                        <span className="text-sm font-medium text-purple-600">#{data.relatedBookingId}</span>
                      </div>
                    )}
                  </div>
                </div>
