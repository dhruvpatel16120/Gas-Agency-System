...
import { Package, DollarSign, Truck, CheckCircle, Clock, Filter, Search, Download, Eye, Edit, Plus, RefreshCw, } from "lucide-react";
...
export default function AdminBookingsPage() {
  ...
  }, [page, limit, statusFilter, methodFilter, paymentFilter, searchQuery, dateFrom, dateTo]);
  ...
  }, [session, loadBookings, loadStats]);
  ...
  const handleBulkAction = async (action: string) => {
    ...
    if (action === "cancel") {
      reason =
        prompt("Please provide a reason for cancellation (required):") || "";
      if (!reason.trim()) {
    ...
    try {
      ...
      if (selected.some((b) => b.status === "DELIVERED")) {
        toast.error(
          "Some selected bookings are already delivered. Service is fulfilled; bulk actions are not allowed for delivered bookings.",
        );
        return;
      }
      if (action === "approve") {
        ...
        if (pendingCount === 0) {
          toast.error(
            "Only PENDING bookings can be approved. No eligible bookings in selection.",
          );
          return;
        }

        // Check UPI payment status - prevent approval if UPI payment is not successful
        const upiBookingsWithoutSuccessPayment = selected.filter(
          (b) =>
            b.status === "PENDING" &&
            b.paymentMethod === "UPI" &&
        ...
        if (upiBookingsWithoutSuccessPayment.length > 0) {
          ...
          toast.error(
            `Cannot approve UPI bookings with pending/failed payments. Booking IDs: ${bookingIds.join(", ")}. Please ensure UPI payments are confirmed before approval.`,
          );
          return;
        }
      }
      ...
      if (action === "assign-delivery") {
        const eligibleAssignCount = selected.filter(
          (b) => b.status === "PENDING" || b.status === "APPROVED",
        ).length;
        if (eligibleAssignCount === 0) {
          toast.error(
            "Only PENDING or APPROVED bookings can be assigned. No eligible bookings in selection.",
          );
          return;
        }
      }
      ...
      if (action === "cancel") {
        const eligibleCancelCount = selected.filter(
      ...
      if (res.ok && result.success) {
        ...
        if (action === "approve") {
          toast.success(
            `Approved ${result.data?.updatedCount || selectedBookings.size} booking(s). Approval emails have been sent to users.`,
          );
        } else if (action === "assign-delivery") {
          toast.success(
            `Assigned delivery partners to ${result.data?.updatedCount || selectedBookings.size} booking(s).`,
          );
        } else if (action === "cancel") {
          toast.success(
            `Cancelled ${result.data?.updatedCount || selectedBookings.size} booking(s). Cancellation emails have been sent to users.`,
          );
        } else {
  ...
  const canBeApproved = (booking: Booking) => {
    ...
    if (booking.paymentMethod === "UPI" && booking.paymentStatus !== "SUCCESS")
      return false;
    return true;
  };
                    ...
                    <button
                      onClick={async () => {
                        ...
                        if (pendingIds.length === 0) {
                          toast.error(
                            "No bookings with pending payment in selection.",
                          );
                          return;
                        }
                        const confirmed = confirm(
                          `Send payment reminder to ${pendingIds.length} booking(s)?`,
                        );
                        if (!confirmed) return;
                        try {
                          ...
                          toast.success(
                            "Payment reminders sent (best effort).",
                          );
                        } catch {
                        ...
                        <tr key={booking.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-3 py-3">
                            <div className="space-y-1">
                              <div className="flex items-center gap-2">
                                <span
                                  className={`px-2 py-1 rounded-full text-xs font-medium ${getPaymentStatusColor(booking.paymentStatus || "PENDING")}`}
                                >
                          ...
                          <td className="px-3 py-3">
                            <div className="flex items-center gap-2">
                              ...
                              <button
                                onClick={() =>
                                  router.push(
                                    `/admin/bookings/${booking.id}/assign-delivery`,
                                  )
                                }
