...
import { ArrowLeft, Save, Package, TrendingUp, TrendingDown, AlertTriangle, FileText, Calendar } from 'lucide-react';
        ...
        <div className="px-4 sm:px-0">
          ...
          <div className="flex items-center gap-4 mb-8">
            ...
            <div>
              ...
              <p className="text-gray-600">Make stock adjustments and corrections</p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
              {/* Quick Adjustments */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-3">Quick Adjustments</label>
                <div className="grid grid-cols-4 gap-2">
                  {quickAdjustments.map((adjustment) => (
                    <button
                      key={adjustment.label}
                      type="button"
                      onClick={() => setFormData(prev => ({ ...prev, delta: adjustment.value.toString() }))}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setFormData(prev => ({ ...prev, delta: e.target.value }))}
                    className="w-full border border-gray-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition-all duration-200"
              ...
              {formData.delta && (
                <div className="bg-gray-50 rounded-xl p-4 border border-gray-200">
                  <h4 className="font-medium text-gray-900 mb-2">Adjustment Summary</h4>
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      <span className="text-gray-600">Type:</span>
                      <span className="ml-2 font-medium text-gray-900">{formData.type}</span>
                    </div>
                    <div>
                      <span className="text-gray-600">Change:</span>
                      <span className={`ml-2 font-medium ${
                        parseInt(formData.delta) >= 0 ? 'text-green-600' : 'text-red-600'
                      }`}>
                        {parseInt(formData.delta) >= 0 ? '+' : ''}{formData.delta} cylinders
                      </span>
                    </div>
                    <div>
                      <span className="text-gray-600">Reason:</span>
                      <span className="ml-2 font-medium text-gray-900">{formData.reason}</span>
                    </div>
                    <div>
                      <span className="text-gray-600">Date:</span>
                      <span className="ml-2 font-medium text-gray-900">{formData.adjustmentDate}</span>
                    </div>
                  </div>
                </div>
              )}
