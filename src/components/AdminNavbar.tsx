...
import { Shield, Menu, LogOut, User, Users, Calendar, Package, Settings, BarChart3, ChevronDown } from 'lucide-react';
 						...
 						{open && (
 							...
 							>
 								...
 								<div className="px-4 pt-4 pb-3 border-b border-gray-100 bg-gradient-to-r from-purple-50 to-white">
 									...
 									<span className="mt-3 inline-flex items-center rounded-full bg-purple-100 px-2.5 py-0.5 text-xs font-medium text-purple-700">Administrator</span>
 								</div>
 								{/* links */}
 								<div className="py-2 text-sm">
 									<Link href="/admin" className="group flex items-center gap-3 px-4 py-2 hover:bg-gray-50" role="menuitem" onClick={() => setOpen(false)}>
 									...
 									<Link href="/admin/bookings" className="group flex items-center gap-3 px-4 py-2 hover:bg-gray-50" role="menuitem" onClick={() => setOpen(false)}>
 										<span className="inline-flex items-center justify-center w-7 h-7 rounded-md bg-purple-50 text-purple-600"><Calendar className="w-4 h-4" /></span>
 										<span className="flex-1">Bookings</span>
 									</Link>
 									<Link href="/admin/users" className="group flex items-center gap-3 px-4 py-2 hover:bg-gray-50" role="menuitem" onClick={() => setOpen(false)}>
 										<span className="inline-flex items-center justify-center w-7 h-7 rounded-md bg-purple-50 text-purple-600"><Users className="w-4 h-4" /></span>
 										<span className="flex-1">Users</span>
 									</Link>
 									<Link href="/admin/inventory" className="group flex items-center gap-3 px-4 py-2 hover:bg-gray-50" role="menuitem" onClick={() => setOpen(false)}>
 									...
 									<Link href="/admin/reports" className="group flex items-center gap-3 px-4 py-2 hover:bg-gray-50" role="menuitem" onClick={() => setOpen(false)}>
 										<span className="inline-flex items-center justify-center w-7 h-7 rounded-md bg-purple-50 text-purple-600"><BarChart3 className="w-4 h-4" /></span>
 				...
 				<div className="md:hidden" id="mobile-nav">
 					...
 					<div className="fixed top-16 inset-x-0 z-50 mx-3 rounded-2xl bg-white text-gray-900 shadow-2xl ring-1 ring-black/5 overflow-hidden">
 						...
 						<nav className="p-2 text-sm">
 							<Link href="/admin" className="block px-4 py-3 rounded-lg hover:bg-gray-50" onClick={() => setMobileOpen(false)}>Dashboard</Link>
 							<Link href="/admin/bookings" className="block px-4 py-3 rounded-lg hover:bg-gray-50" onClick={() => setMobileOpen(false)}>Bookings</Link>
 						...
 						<div className="border-t border-gray-100 p-2">
 							<button
 								...
 								onClick={async () => { setMobileOpen(false); await signOut({ callbackUrl: '/admin/login' }); }}
 							>
 								Sign Out
 							</button>
 						</div>
