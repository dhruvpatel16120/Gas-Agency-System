...
async function createOrUpdateAdmin() {
  ...
  try {
    ...
    let password = '';
    while (true) {
      const p1 = await askQuestion(colorize('Password (min 8 chars, include upper/lower/number/special)', 'blue'));
      ...
      if (!/[!@#$%^&*(),.?":{}|<>]/.test(p1)) errors.push('one special character');
      if (errors.length) {
    ...
    console.log(colorize('Note: The password is shown for confirmation only; it is stored hashed in the database.', 'dim'));
